...
import com.alvin.health.model.UserInfo;
...
//        return m.matches();
    ...
    /**
     * Use an AsyncTask to fetch the user's phone on a background thread, and update
     * the phone text field with results on the main UI thread.
     */
    class SetupPhoneAutoCompleteTask extends AsyncTask<Void, Void, List<String>> {

        @Override
        protected List<String> doInBackground(Void... voids) {
            ArrayList<String> phoneCollection = new ArrayList<>();

            // Get all phone from the user's contacts and copy them to a list.
            SharedPreferences sharedPreferences = getSharedPreferences("phone", Context.MODE_PRIVATE);
            Map<String, ?> allPhone =  sharedPreferences.getAll();
            if(allPhone.size()>0)
            {
                for(Map.Entry<String, ?>  entry : allPhone.entrySet()){
                    phoneCollection.add(entry.getKey());
                }
            }
        ...
        @Override
        protected void onPostExecute(List<String> phoneCollection) {
            addPhoneToAutoComplete(phoneCollection);
        }
    }

    private void addPhoneToAutoComplete(List<String> phoneCollection) {
